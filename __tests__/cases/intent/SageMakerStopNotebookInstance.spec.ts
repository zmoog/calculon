...
import { SageMakerStopNotebookInstance } from "../../../scripts/intent/SageMakerStopNotebookInstance";


describe("SageMakerStopNotebookInstance, given a Jupyter Notebook Instance", () => {

    // let togglService: IMock<ITogglService>;
    let intent: SageMakerStopNotebookInstance; 

    beforeEach(() => {
        // togglService = Mock.ofType<ITogglService>();
        // intent = new TogglSummaryIntent(togglService.object);
        intent = new SageMakerStopNotebookInstance();
    });

    describe("when the jupyter notebook instance is 'running' and has the AutoShutDown set to 'on'", () => {
        beforeEach(() => {
        ...
        it("should stop the instance", async () => {
            ...
            expect(response).to.be.eql(`Stopping 1 notebook instance.`);
